...
export default function ColorPalette() {
  return (
    <div class="flex flex-wrap justify-center gap-3 p-4 bg-white rounded-lg shadow-sm">
      <For each={gameState.currentColors()}>
        {(color) => (
          <button
            ...
            style={{ 'background-color': colors[color as keyof typeof colors] }}
            onClick={() => gameState.selectColor(color)}
            disabled={gameState.success() || gameState.over() || gameState.currentAttempt().includes(color)}
